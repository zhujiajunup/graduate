...
$(function () {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            var data = [];
            for (var i = 0; i < wordCountData.length; i++) {
                data[i] = {
                    name: wordCountData[i]['word'],
                    value: wordCountData[i]['count'],
                    itemStyle: {
                        normal: {
                            color: 'rgb(' + [
                                Math.round(Math.random() * 160),
            ...
            var myChart = echarts.init(document.getElementById("flot-line-word-cloud"));
            myChart.setOption(option);
        }
    });
});
